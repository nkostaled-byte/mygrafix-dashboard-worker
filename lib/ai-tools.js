...
// Per-client index of the single pending action awaiting confirmation, so the
// chat handler can recover it server-side without relying on the frontend.
const ACTION_PENDING_PREFIX = "ai_pending:";
...
export async function executeTool(toolName, args, env, clientId) {
  ...
  if (isWriteTool(toolName)) {
    ...
    return {
      ...
      message: `The system will automatically display a confirmation card with Confirm/Cancel buttons for this ${action.label.toLowerCase()}. Your reply to the user must be EMPTY or a single short neutral sentence. Do NOT summarize the details again. Do NOT say "Confirm below" or "Shall I proceed?" or ask the user to type anything. The buttons are the only confirmation mechanism.`,
    };
  }
...
export async function createPendingAction(toolName, args, clientId, env) {
  ...
  // Best-effort resolution of customer/service IDs (client-scoped) so the
  // pending action carries a complete, safe payload for execution. Execution
  // re-validates ownership regardless of what is stored here.
  let resolvedIds = {};
  if (toolName === "create_booking") {
    try {
      const customers = await supabaseFetch(
        env,
        `customers?client_id=eq.${encodeURIComponent(clientId)}&name=eq.${encodeURIComponent(args.customerName)}&select=id&limit=1`
      );
      resolvedIds.customer_id = (customers && customers[0] && customers[0].id) || null;
      const services = await supabaseFetch(
        env,
        `services?client_id=eq.${encodeURIComponent(clientId)}&name=ilike.${encodeURIComponent(args.serviceName)}&select=id,price,duration_minutes&limit=1`
      );
      resolvedIds.service_id = (services && services[0] && services[0].id) || null;
    } catch {
      // Non-fatal: execution will re-resolve and validate.
    }
  }
  ...
  const action = {
    ...
    rawFields: { ...config.rawFields, ...resolvedIds },
    clientId,
    createdAt: now,
    executed: false,
  };

  // Store in KV with 5-minute TTL and index it by client so the chat handler
  // can recover the current pending action without the frontend. Replace any
  // previous pending action for this client (one at a time).
  if (env.RATE_LIMIT_KV) {
    const prevId = await env.RATE_LIMIT_KV.get(ACTION_PENDING_PREFIX + clientId).catch(() => null);
    if (prevId && prevId !== actionId) {
      await env.RATE_LIMIT_KV.delete(ACTION_KV_PREFIX + prevId);
    }
    await env.RATE_LIMIT_KV.put(ACTION_KV_PREFIX + actionId, JSON.stringify(action), { expirationTtl: 300 });
    await env.RATE_LIMIT_KV.put(ACTION_PENDING_PREFIX + clientId, actionId, { expirationTtl: 300 });
  }
...
export async function getPendingAction(actionId, clientId, env) {
  ...
  if (Date.now() - action.createdAt > ACTION_TTL_MS) {
    ...
    await env.RATE_LIMIT_KV.delete(ACTION_PENDING_PREFIX + action.clientId);
    return null;
  }
  return action;
}
...
/**
 * Get the single pending action currently awaiting confirmation for a client.
 * Returns null if there is none, it expired, or was already executed.
 *
 * @param {string} clientId - Server-resolved client ID
 * @param {object} env - Worker env bindings (for KV access)
 * @returns {Promise<object|null>} The pending action, or null
 */
export async function getPendingActionForClient(clientId, env) {
  if (!env.RATE_LIMIT_KV) return null;

  const actionId = await env.RATE_LIMIT_KV.get(ACTION_PENDING_PREFIX + clientId);
  if (!actionId) return null;

  return getPendingAction(actionId, clientId, env);
}

/**
 * Mark a pending action as executed (delete from KV to prevent reuse).
 */
export async function markActionExecuted(action, env) {
  if (!env.RATE_LIMIT_KV) return;
  await env.RATE_LIMIT_KV.delete(ACTION_KV_PREFIX + action.id);
  if (action.clientId) {
    await env.RATE_LIMIT_KV.delete(ACTION_PENDING_PREFIX + action.clientId);
  }
}
...
async function executeCreateBooking(fields, env, clientId) {
  ...
  if (!fields.customerName || !fields.serviceName || !fields.date || !fields.time) {
    return { error: "Booking details are incomplete. Customer, service, date, and time are required." };
  }

  // 2. Validate date/time format and parseability
  if (!/^\d{4}-\d{2}-\d{2}$/.test(fields.date) || !/^([01]\d|2[0-3]):[0-5]\d$/.test(fields.time)) {
    return { error: "Invalid booking date or time format." };
  }
  const startDate = new Date(`${fields.date}T${fields.time}:00`);
  if (isNaN(startDate.getTime())) {
    return { error: "Invalid booking date or time." };
  }

  // 3. Resolve the customer (client-scoped) and validate ownership
  let customerId = fields.customer_id || null;
  if (customerId) {
    const owned = await supabaseFetch(
      env,
      `customers?id=eq.${encodeURIComponent(customerId)}&client_id=eq.${encodeURIComponent(clientId)}&select=id&limit=1`
    );
    if (!owned || !owned.length) {
      return { error: "Customer does not belong to your business." };
    }
  } else {
    const customers = await supabaseFetch(
      env,
    ...
    if (customers && customers.length) {
      ...
      if (!customerId) return { error: "Failed to resolve the customer for this booking." };
    }
  }

  // 4. Resolve the service (client-scoped) and validate ownership
  let serviceId = fields.service_id || null;
  let durationMinutes = 30;
  let servicePrice = 0;
  if (serviceId) {
    const owned = await supabaseFetch(
      env,
      `services?id=eq.${encodeURIComponent(serviceId)}&client_id=eq.${encodeURIComponent(clientId)}&select=id,price,duration_minutes&limit=1`
    );
    if (!owned || !owned.length) {
      return { error: "Service does not belong to your business." };
    }
    durationMinutes = owned[0].duration_minutes || 30;
    servicePrice = owned[0].price || 0;
  } else {
    const services = await supabaseFetch(
      env,
  ...
  if (!serviceId) {
    return { error: `Service "${fields.serviceName}" was not found in your services. Add it first or try a different service.` };
  }

  // 5. Conflict check: reject if another booking overlaps this time slot
  ...
  const conflicts = await supabaseFetch(
    env,
    `bookings?client_id=eq.${encodeURIComponent(clientId)}&start_time=lt.${encodeURIComponent(endTime)}&end_time=gt.${encodeURIComponent(startTime)}&status=neq.cancelled&select=id&limit=1`
  );
  if (conflicts && conflicts.length) {
    return { error: "There is already a booking at that time." };
  }
  ...
  const [booking] = await supabaseFetch(env, "bookings", {
    ...
    body: JSON.stringify({
      ...
      end_time: endTime,
      status: "upcoming",
      amount: fields.amount || servicePrice || 0,
...
export function buildBookingResponse(booking) {
 ...
 */
export function generateConfirmationReply(action, result) {
  switch (action.type) {
    case "create_booking": {
      const fields = action.fields;
      const date = formatBookingDate(fields.date || "");
      const time = formatBookingTime(fields.time || "");
      return `Booking confirmed. ${fields.customer}'s ${fields.service} has been booked for ${date} at ${time}.`;
    }
    case "create_customer": {
