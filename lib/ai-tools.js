...
// ─── Pending Actions Store (in-memory, per-worker-instance) ────────
...
export async function executeTool(toolName, args, env, clientId) {
  ...
  if (isWriteTool(toolName)) {
    const action = createPendingAction(toolName, args, clientId);
    if (!action) {
 ...
 * @returns {object|null} Pending action object, or null if validation fails
 */
export function createPendingAction(toolName, args, clientId) {
 ...
 * @returns {object|null} The pending action, or null if invalid
 */
export function getPendingAction(actionId, clientId) {
