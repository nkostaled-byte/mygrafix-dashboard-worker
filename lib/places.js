...
export async function searchPlaces(env, { query, location, limit } = {}) {
  ...
  if (!apiKey) {
    console.error("[Places API] GOOGLE_PLACES_API_KEY is not configured");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[Places API]", { status: res.status, body: text, query, location });
      return { error: msg + (env.DEBUG ? ` — ${text}` : "") };
    }

    const json = await res.json();
    const places = (json.places || []).map(normalizePlace).filter(Boolean);
    console.log("[Places API] Success:", { query, location, count: places.length });
    return { data: places };
  } catch (err) {
    console.error("[Places API] Exception:", err?.message, err?.name);
    return {
      error:
