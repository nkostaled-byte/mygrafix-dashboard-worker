...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  // ─── HEADER ────────────────────────────────────────────────────────────
  // Logo + business name + contact (left) | INVOICE + meta (right)
  ...
  const nameX = logoImage ? contentLeft + 130 : contentLeft;
  ...
  // Meta row: Invoice No | Issued | Due (right-aligned, 3 columns)
  const metaColW = 90;
  const metaItems = [
  ...
  const totalTextWidth = boldFont.widthOfTextAtSize(totalText, 13);
  page.drawText(totalText, {
    x: contentRight - totalBoxW + (totalBoxW - totalTextWidth) / 2,
    y: y - totalBoxH + (totalBoxH - 13) / 2 + 3,
    size: 13,
    font: boldFont,
    color: WHITE,
  });
  ...
  // ── PAYMENT TERMS (card on right) ─────────────────────────────────────
  const termsCardW = 220;
  const termsCardH = 90;
