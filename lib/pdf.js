...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  const primaryColor = hexToRgb(client.primary_color) || TEXT_COLOR;
  const accentColor = hexToRgb(client.secondary_color) || LINE_COLOR;
  ...
  page.drawText("Qty", { x: col.qty, y, size: 9, font: boldFont, color: primaryColor });
  page.drawText("Price", {
    x: col.price,
    y,
    size: 9,
  ...
  y -= 20;
  drawRightAligned(page, boldFont, `Total   ${formatMoney(invoice.total)}`, y, 13, primaryColor);
