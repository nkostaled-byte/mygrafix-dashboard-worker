...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  // BILL TO
  drawRect(page, L, y - cardH, cardW, cardH, 8, WHITE, LIGHT_GRAY);
  page.drawText("Bill To", { x: L + 16, y: y - 20, size: 10, font: bold, color: brand });
  page.drawText(customer.name || "", { x: L + 16, y: y - 38, size: 14, font: bold, color: BLACK });
  page.drawText(customer.email || "", { x: L + 16, y: y - 56, size: 10, font, color: MED_GRAY });

  // FROM
  const fromX = M + gap / 2;
  drawRect(page, fromX, y - cardH, cardW, cardH, 8, WHITE, LIGHT_GRAY);
  page.drawText("From", { x: fromX + 16, y: y - 20, size: 10, font: bold, color: brand });
  page.drawText(client.business_name || "", { x: fromX + 16, y: y - 38, size: 14, font: bold, color: BLACK });
  ...
  // Header
  drawRect(page, L, y, W, headerH, radius, brand);
  const hY = y - 24;
  ...
  // Grand total box
  const boxH = 52;
  drawRect(page, totX, y - boxH, totW, boxH, 8, brand);
  page.drawText("Total", { x: totX + 16, y: y - 34, size: 14, font: bold, color: WHITE });
  drawRight(page, formatMoney(invoice.total), R - 16, y - 34, 16, bold, WHITE);
  ...
  if (hasBank) {
    const bankH = 130;
    drawRect(page, L, y - bankH, W, bankH, 8, WHITE, LIGHT_GRAY);

    page.drawText("Banking Details", { x: L + 16, y: y - 22, size: 10, font: bold, color: brand });
  ...
  if (client.payment_instructions || invoice.due_at) {
    ...
    page.drawText("Payment Terms", { x: termsX + 16, y: y - 22, size: 10, font: bold, color: brand });
