 ...
 * Table borders/accents use the same client primary color.
 *
 * Exports:
...
const NEUTRAL_FALLBACK = rgb(17 / 255, 24 / 255, 39 / 255); // #111827 (only if primary_color missing)
...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  const ACCENT_ICON_BG = tint(ACCENT, 0.88);
  const ACCENT_CARD_BG = tint(ACCENT, 0.93);
  const ACCENT_CARD_BD = tint(ACCENT, 0.78);
  const STRIPE         = tint(ACCENT, 0.97);
  const ROW_LINE       = tint(ACCENT, 0.85); // table divider hairline (brand-derived)
  ...
  const lineCount = (str, size, maxWidth) =>
    Math.max(1, Math.ceil(width(str, size) / maxWidth));
  ...
  /* ══ BILL TO / FROM CARDS — content-based height ══════════════════════ */
  const cardW = (CW - 20) / 2;

  const billLines = (customer.email ? 1 : 0) + (customer.phone ? 1 : 0);
  const addrLines = client.address ? lineCount(client.address, 9, cardW - 74) : 0;
  const fromLines = addrLines + (client.phone ? 1 : 0) + (client.email ? 1 : 0);
  const maxLines = Math.max(billLines, fromLines, 1);
  const cardH = Math.max(96, 70 + 15 * (maxLines - 1) + 12);
  ...
  /* ══ LINE ITEMS TABLE — rounded, brand-colored border ═════════════════ */
  const headerH = 26;
  const rowH = 28; // measured: 32pt reserved 22pt of padding for 9.5pt text; 28pt keeps 18pt — still comfortable
  const R = 8;
  let tableSegTop = y;
  ...
  const closeTableSegment = (roundedBottom) => {
    ...
    try {
      page.drawSvgPath(rrPath(CW, h, R, R, rB, rB), {
        x: MX, y: tableSegTop, borderColor: ACCENT, borderWidth: 0.8, // brand border (was grey)
      });
    } catch (e) {
      page.drawRectangle({ x: MX, y: tableSegTop - h, width: CW, height: h, borderColor: ACCENT, borderWidth: 0.8 });
    }
  };
  ...
  if (!lineItems.length) {
    ...
    text("No items", TBL.descX, y - 18, { size: 9.5, color: MID_GRAY });
    y -= rowH;
  }
  ...
  lineItems.forEach((item, idx) => {
    ...
    text(truncate(item.description || "Item", 62), TBL.descX, rowTop - 18, { size: 9.5 });
    textRight(String(item.quantity || 0), TBL.qtyRight, rowTop - 18, { size: 9.5 });
    textRight(formatCurrency(item.unit_price, currency), TBL.unitRight, rowTop - 18, { size: 9.5 });
    const lt = Number(item.line_total ?? (Number(item.quantity || 0) * Number(item.unit_price || 0)));
    textRight(formatCurrency(lt, currency), TBL.totalRight, rowTop - 18, { size: 9.5, f: bold });

    y -= rowH;
    if (!isLast) line(MX + 1, y, RIGHT - 1, y, 0.5, ROW_LINE);
  });
  ...
  /* ══ BANKING + PAYMENT TERMS — exact-space guard ══════════════════════ */
  // Real space required below this point:
  //   terms card bottom = sectionTop − 111  (7 offset + 118 card… card top +7, height 118)
  //   + 12pt breathing gap above the footer line (y = 120).
  // No extra safety area reserved.
  const BANKING_FLOOR = 120 + 12 + 111; // = 243
  if (y < BANKING_FLOOR) startPage(true);
  ...
  icon("calendar", ptX + 14, ptTitleY + 3, 15, ACCENT);
...
/* ═══════════════════ WEBSITE AUDIT REPORT (leads) ═══════════════════ */

/**
 * Branded website audit report for a sales prospect.
 * @param {object} client - the authenticated client row (brand color + name)
 * @param {object} data - { businessName, url, domain, score, opportunityLevel,
 *                         priority, deductions, recommendedServices,
 *                         salesMessage, emails, phones, address, date }
 */
export async function generateAuditReportPdf(client, data) {
  const pdfDoc = await PDFDocument.create();
  const font = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const bold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);

  const ACCENT     = ensureContrast(hexToRgbColor(client?.primary_color, NEUTRAL_FALLBACK));
  const ACCENT_BG  = tint(ACCENT, 0.93);
  const ACCENT_BD  = tint(ACCENT, 0.8);
  const STRIPE     = tint(ACCENT, 0.97);

  const agencyName = sanitizeForPdf(client?.business_name || "My Grafix Media");
  data = data || {};
  ...
  const text = (str, x, yPos, o = {}) => {
    const { size = 9, f = font, color = DARK, maxWidth, lineHeight } = o;
    const opts = { x, y: yPos, size, font: f, color };
    if (maxWidth) { opts.maxWidth = maxWidth; opts.lineHeight = lineHeight || size * 1.45; }
    try { page.drawText(sanitizeForPdf(str), opts); }
    catch (e) { try { page.drawText(sanitizeForPdf(str).replace(/[^\x00-\x7F]/g, ""), opts); } catch (e2) {} }
  };
  const width = (str, size, f = font) => f.widthOfTextAtSize(sanitizeForPdf(str), size);
  const textRight = (str, xRight, yPos, o = {}) => text(str, xRight - width(str, o.size || 9, o.f || font), yPos, o);
  const line = (x1, y1, x2, y2, thickness = 0.6, color = LIGHT_LINE, dash) => {
    const o = { start: { x: x1, y: y1 }, end: { x: x2, y: y2 }, thickness, color };
    if (dash) o.borderDashArray = dash;
    page.drawLine(o);
  };
  const rect = (x, yTop, w, h, color) => page.drawRectangle({ x, y: yTop - h, width: w, height: h, color });
  const roundedRect = (x, yTop, w, h, r, o = {}) => {
    try {
      page.drawSvgPath(rrPath(w, h, r, r, r, r), { x, y: yTop, color: o.color, borderColor: o.borderColor, borderWidth: o.borderWidth || 0 });
    } catch (e) {
      if (o.color) rect(x, yTop, w, h, o.color);
    }
  };
  const wrapped = (str, x, yPos, size, maxWidth, color, f = font) => {
    const words = sanitizeForPdf(str).split(/\s+/).filter(Boolean);
    let lineStr = "";
    for (const word of words) {
      const attempt = lineStr ? `${lineStr} ${word}` : word;
      if (width(attempt, size, f) > maxWidth && lineStr) {
        text(lineStr, x, yPos, { size, color, f });
        yPos -= size * 1.45;
        lineStr = word;
      } else {
        lineStr = attempt;
    ...
    if (lineStr) { text(lineStr, x, yPos, { size, color, f }); yPos -= size * 1.45; }
    return yPos;
  };
  const newPage = () => {
    page = pdfDoc.addPage([W, H]);
    rect(0, H, W, 8, ACCENT);
    y = H - 48;
    text("Website Audit Report (continued)", MX, y, { size: 10, f: bold, color: ACCENT });
    y -= 24;
  };
  const ensure = (space) => { if (y - space < 70) newPage(); };

  /* ── Header ── */
  rect(0, H, W, 10, ACCENT);
  text("Website Audit Report", MX, H - 58, { size: 20, f: bold });
  text(`Prepared by ${agencyName}`, MX, H - 80, { size: 9, color: GRAY });
  const dateStr = new Date(data.date || Date.now()).toLocaleDateString("en-ZA", { day: "numeric", month: "long", year: "numeric" });
  textRight(dateStr, RIGHT, H - 58, { size: 9, color: GRAY });
  textRight(truncate(data.url || data.domain || "", 46), RIGHT, H - 80, { size: 8, color: MID_GRAY });

  y = H - 118;

  /* ── Business + score band ── */
  ensure(120);
  roundedRect(MX, y, CW, 92, 12, { color: STRIPE, borderColor: ACCENT_BD, borderWidth: 1 });
  text(truncate(data.businessName || data.domain || "Prospect", 44), MX + 18, y - 26, { size: 14, f: bold });
  text(truncate(data.url || data.domain || "", 66), MX + 18, y - 44, { size: 9, color: GRAY, maxWidth: CW - 220 });

  text(String(data.score ?? 0), RIGHT - 132, y - 52, { size: 32, f: bold, color: ACCENT });
  text("/ 100", RIGHT - 92, y - 52, { size: 9, color: GRAY });
  text(`${data.opportunityLevel || "—"} opportunity · priority ${data.priority || "—"}`, RIGHT - 150, y - 70, { size: 8, color: GRAY, maxWidth: 150 });

  y -= 116;

  /* ── AI sales message ── */
  if (data.salesMessage) {
    ensure(110);
    text("Ready-to-send sales message", MX, y, { size: 10, f: bold, color: ACCENT });
    y -= 16;
    roundedRect(MX, y, CW, 20 + Math.ceil(width(data.salesMessage, 9) / (CW - 36)) * 13, 8, { color: CARD_BG, borderColor: CARD_BORDER, borderWidth: 0.8 });
    y = wrapped(data.salesMessage, MX + 14, y - 18, 9, CW - 36, GRAY) - 12;
  }

  /* ── Recommended services ── */
  if (data.recommendedServices && data.recommendedServices.length) {
    ensure(60 + data.recommendedServices.length * 16);
    text("Recommended services", MX, y, { size: 10, f: bold, color: ACCENT });
    y -= 18;
    for (const s of data.recommendedServices) {
      text(`•  ${typeof s === "string" ? s : s.name || ""}`, MX + 6, y, { size: 9, color: GRAY });
      y -= 15;
    }
    y -= 8;
  }
  ...
  if (data.deductions && data.deductions.length) {
    ensure(60 + data.deductions.length * 16);
    text("Score deductions", MX, y, { size: 10, f: bold, color: ACCENT });
    y -= 18;
    for (const d of data.deductions) {
      text(d.label || "—", MX + 6, y, { size: 9, color: GRAY });
      textRight(`-${d.points}`, RIGHT, y, { size: 9, f: bold, color: rgb(0.85, 0.2, 0.25) });
      y -= 15;
    }
    y -= 8;
  }
  ...
  if (contacts.length) {
    ensure(50 + contacts.length * 30);
    text("Contact details", MX, y, { size: 10, f: bold, color: ACCENT });
    y -= 18;
    for (const [label, value] of contacts) {
      text(`${label}: ${value}`, MX + 6, y, { size: 9, color: GRAY, maxWidth: CW - 12, lineHeight: 12 });
      y -= 28;
    }
  }

  /* ── Footer ── */
  line(MX, 96, RIGHT, 96, 0.8, LIGHT_LINE, [4, 3]);
  text(`Report generated ${dateStr} by ${agencyName} using My Business OS.`, MX, 76, { size: 8, color: MID_GRAY });
