 ...
 * Clean cursor-based layout system
 */
...
function drawPartyCard(page, fonts, brand, x, y, width, height, title, name, lines, iconType) {
  ...
  drawRounded(page, x, cardBottom, width, height, 9, {
    color: WHITE,
    borderColor: BORDER,
    borderWidth: 1,
  });

  // Icon position (inside card, near top-left)
  const iconSize = 38;
  const iconX = x + 16;
  const iconY = cardBottom + height - 16 - iconSize;
  ...
  // Text position (to the right of icon, inside card)
  const tx = iconX + iconSize + 16;
  const textTop = y - 22;
  page.drawText(title, { x: tx, y: textTop, size: 9.5, font: bold, color: brand });
...
function drawTableHeader(page, fonts, brand, cursorY) {
  const { bold } = fonts;
  const h = 34;
  const headerBottom = cursorY - h;
  drawRounded(page, ML, headerBottom, CW, h, 8, { color: brand });

  const { descriptionX, quantityX, unitPriceX, totalX, widths } = columnGeometry();
  const textY = headerBottom + 11;
  ...
  return { nextY: headerBottom, height: h };
}
...
function drawItemRow(page, fonts, item, index, cursorY, rowH) {
  ...
  if (index % 2 === 1) {
    page.drawRectangle({ x: ML, y: rowBottom, width: CW, height: rowH, color: SOFT });
  }

  const lines = wrapText(font, item.description, 10.5, widths.description - 24);
  const textBlockH = lines.length * 13;
  let textY = rowBottom + (rowH + textBlockH) / 2 - 9;
  ...
  const baseline = rowBottom + rowH / 2 - 4;
  ...
  page.drawLine({
    start: { x: ML, y: rowBottom },
    end: { x: MR, y: rowBottom },
    thickness: 0.45,
    color: BORDER,
  });
...
function drawTotals(page, fonts, brand, invoice, cursorY) {
  ...
  drawRounded(page, x, boxBottom, width, boxH, 9, { color: brand });
  page.drawText("Total", { x: x + 16, y: boxBottom + 17, size: 16, font: bold, color: WHITE });
  drawRight(page, formatMoney(invoice.total), MR - 16, boxBottom + 15, 17, bold, WHITE);
...
function drawBankingAndPayment(page, fonts, brand, brandLight, client, invoice, cursorY) {
  ...
  // Banking Details title and line
  ...
  drawRounded(page, paymentX, cardBottom, paymentW, cardH, 9, {
    color: brandLight,
    borderColor: BORDER,
    borderWidth: 1,
  });
