...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  // Capture topY BEFORE drawing banking — Payment Terms card aligns to this same edge.
  const sectionTop = y;
  ...
  // --- Payment terms card (right) — aligned to SAME top as Banking Details ---
  const ptW = 160;
  const ptX = RIGHT - ptW;
  const ptH = 118;
  const ptTop = sectionTop;
  roundedRect(ptX, ptTop, ptW, ptH, 8, { color: TERMS_BG, borderColor: TERMS_BORDER, borderWidth: 0.8 });
  ...
  line(MX, footerDividerY, RIGHT, footerDividerY, 0.8, rgb(0.8, 0.8, 0.8), [4, 3]);

  text("Thank you!", MX, footerDividerY - 22, { size: 10, f: bold, color: GREEN });
  text("We appreciate your business.", MX, footerDividerY - 38, { size: 9, color: GRAY });

  const brand = "My Grafix Media";
  const poweredPlain = "Powered by ";
  const w1 = width(poweredPlain, 9);
  const w2 = width(brand, 9, bold);
  text(poweredPlain, RIGHT - w1 - w2, footerDividerY - 22, { size: 9, color: GRAY });
  text(brand, RIGHT - w2, footerDividerY - 22, { size: 9, f: bold, color: GREEN });
  textRight("Design. Build. Automate.", RIGHT, footerDividerY - 38, { size: 9, color: MID_GRAY });
