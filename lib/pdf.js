 ...
 * Used for the "light brand tint" table header and card backgrounds.
 */
...
export async function generateInvoicePdf(client, invoice, lineItems, customer) {
  ...
  const brandLight = lightenHex(client.primary_color, 0.9); // Very light tint for cards
  ...
  // ---- Meta row (Invoice No / Issued / Due) — fixed columns ----
  const metaColW = 92;
  const metaItems = [
  ...
  // FROM card
  drawRoundedRect(page, colMid + cardGap / 2, y - cardHeight, cardWidth, cardHeight, 8, WHITE, LINE_COLOR, 1);
  page.drawText("FROM", {
    x: colMid + cardGap / 2 + 12,
    y: y - 18,
    size: 9,
    font: boldFont,
    color: brand,
