...
export async function supabaseFetch(env, path, options = {}) {
  ...
  console.log("SERVICE_ROLE_KEY exists:", !!env.SUPABASE_SERVICE_ROLE_KEY);
...
/**
 * Create (or fetch) a Supabase Auth user via the Admin API (service role).
 * Creates the user with a confirmed email so they can sign in immediately.
 * If the email is already registered, fetches and returns the existing user.
 * @param {object} env
 * @param {{email: string, password: string, fullName?: string}} input
 * @returns {Promise<{user: object, alreadyExists: boolean}>}
 */
export async function supabaseAdminCreateUser(env, { email, password, fullName }) {
  const baseUrl = (env.SUPABASE_URL || "").replace(/\/$/, "");
  const authHeaders = {
    apikey: env.SUPABASE_SERVICE_ROLE_KEY,
    Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
  ...
  if (createRes.status === 422) {
    // User already registered — look them up by email and reuse the account
    const lookupRes = await fetch(`${baseUrl}/auth/v1/admin/users?email=${encodeURIComponent(email)}`, {
      headers: { apikey: env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}` },
    });
    if (lookupRes.ok) {
      const list = await lookupRes.json();
      if (list.users && list.users.length) {
        return { user: list.users[0], alreadyExists: true };
      }
    }
    throw new Error(`That email is already registered but could not be linked.`);
  }

  if (!createRes.ok) {
    const text = await createRes.text().catch(() => "Unknown error");
    throw new Error(`Failed to create account (${createRes.status}): ${text}`);
  }
...
class SupabaseError extends Error {  constructor(message, statusCode) {
    super(message);
